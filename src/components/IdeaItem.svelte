...
<li class="bg-white shadow rounded-lg p-4">
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center space-x-4">
      <button onclick={() => handleUpvote(idea.id)} class="flex items-center space-x-1 text-gray-600 hover:text-blue-500">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd" d="M3.293 9.707a1 1 0 010-1.414l6-6a1 1 0 011.414 0l6 6a1 1 0 01-1.414 1.414L11 5.414V17a1 1 0 11-2 0V5.414L4.707 9.707a1 1 0 01-1.414 0z" clip-rule="evenodd" />
        </svg>
        <span>{idea.votes}</span>
      </button>
      <button onclick={() => handleValueVote(idea.id)} class="flex items-center space-x-1 text-gray-600 hover:text-green-500">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
