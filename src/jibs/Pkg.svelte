<script>
    ...
    const bun_packages = ["@343dev/optimizt", "wrangler"];
    ...
    const packagesByInstaller = $derived({
        ...
        uv: Array.from(selectedPackages).filter(
            (pkg) => pkg.installer === "uv",
        ),
        mas: Array.from(selectedPackages).filter(
            (pkg) => pkg.installer === "mas",
        ),
        bun: Array.from(selectedPackages).filter(
            (pkg) => pkg.installer === "bun",
        ),
        direct: Array.from(selectedPackages).filter((pkg) =>
            direct_from_curl.some((d) => d.name === pkg.installer),
        ),
    });
    ...
    const packageCommands = $derived({
        ...
        mas:
            packagesByInstaller.mas.length > 0
                ...
                : null,
        bun:
            packagesByInstaller.bun.length > 0
        ...
        <Card.Content>
            ...
            <div class="package-list">
                {#each [...packages].sort( (a, b) => display(a).localeCompare(display(b)), ) as pkg}
                    <button
                        class="package-name {selectedPackages.has(pkg)
                            ? 'selected'
                            : ''}"
                        onpointerdown={(e) => {
                        ...
                        onpointerenter={(e) => {
                            if (e.buttons > 0) {
                                // check if button is pressed during hover
                                togglePackage(pkg);
                            }
                        }}
                    >
                    ...
                    {#if groupedCommands.installers.length > 0}
                        <div class="command-section">
                            <div class="code-container">
                                ...
                                <pre><code
                                        >{groupedCommands.installers.join(
                                            "\n",
                                        )}</code
                                    ></pre>
                            </div>
