<script>
    ...
    function toggleRange(startIndex, endIndex) {
        ...
        const allSelected = unitsInRange.every((unit) =>
            selected_units.includes(unit),
        );

        if (allSelected) {
            // If all selected, deselect them (but prevent removing all units)
            const remainingUnits = selected_units.filter(
                (unit) => !unitsInRange.includes(unit),
            );
            ...
            const newSelected = [
                ...new Set([...selected_units, ...unitsInRange]),
            ];
    ...
    function handleTouchMove(e) {
        if (isDragging) {
            ...
            const unitItem = elements.find((el) =>
                el.classList.contains("unit-item"),
            );
            if (unitItem) {
                const index = Array.from(
                    unitItem.parentElement.children,
                ).indexOf(unitItem);
    ...
    const salary = Object.fromEntries(
        Object.entries(timeUnits).map(([unit, multiplier]) => [
            unit,
            {
                ...
                set: (value) => {
                    const inputValue =
                        parseFloat(value?.target?.value ?? value) || 0;
        ...
        <div class="sidebar">
            ...
            <div class="unit-selector-sidebar">
                {#each orderedUnitKeys as unit, i}
                    <div
                        class="unit-item {selected_units.includes(unit)
                            ? 'active'
                            : ''} {isDragging &&
...
<style>
    ...
    .input {
        font-family:
            "Roboto Mono", "SF Mono", "Fira Code", "Fira Mono", "Menlo",
            monospace;
        font-size: 1.5rem;
        font-weight: 300;
        padding: 0.5rem 0;
