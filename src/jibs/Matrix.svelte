<script>
    ...
    export const CHAR_SETS = {
        latin: [
            "⋅",
            ...charRange(0x0041, 0x005a),
            ...charRange(0x0061, 0x007a),
        ],
        ...
        cyrillic: [
            "⋅",
            ...charRange(0x0410, 0x042f),
            ...charRange(0x0430, 0x044f),
        ],
    ...
    function createStream(isDecrypted = false) {
        ...
        if (isDecrypted) {
            const decryptedText =
                decryptedStrings[
                    Math.floor(Math.random() * decryptedStrings.length)
                ];
    ...
    function getCharColor(char, stream, timestamp, baseAlpha) {
        const adjustedBaseAlpha =
            0.4 + (stream.depthLevel / (STREAM.DEPTH_LEVELS - 1)) * 0.6;

        if (!stream.decrypted) {
            if (char.isRotating) {
                const progress = Math.min(
                    1,
                    (timestamp - char.rotateStartTime) / 200,
                );
    ...
    function updateStream(stream, timestamp) {
        ...
        if (!stream.decrypted) {
            ...
            if (!stream.isFullyRevealed) {
                if (
                    timestamp - stream.lastCharRevealTime >
                    stream.charRevealDelay
                ) {
                    if (stream.nextRevealIndex < stream.chars.length) {
                        // Reveal from bottom up
                        const revealIndex =
                            stream.chars.length - 1 - stream.nextRevealIndex;
            ...
            // Rotation phase - only start after stream is fully revealed
                ...
                char.char =
                    char.charSet[
                        Math.floor(Math.random() * char.charSet.length)
                    ];
                char.isRotating = true;
                char.rotateStartTime = timestamp;
        ...
        if (!stream.decrypted) {
            ...
            if (
                timestamp >= stream.nextRotationTime &&
                stream.nextRotationIndex < stream.chars.length
            ) {
                const char = stream.chars[stream.nextRotationIndex];
                ...
                if (char.isVisible) {
                    ...
                    stream.nextRotationIndex =
                        (stream.nextRotationIndex + 1) % stream.chars.length;
        ...
        function draw(timestamp) {
            ...
            if (streams.size < STREAM.MAX_STREAMS) {
                ...
                // Regular stream creation logic
                    ...
                    timeSinceLastStream >
                    getRandomValue(
                        STREAM.CREATION_GAP_MIN,
                        STREAM.CREATION_GAP_MAX,
                    )
                ) {
                    ...
                    const targetDensity =
                        STREAM.TARGET_DENSITY +
                        (Math.random() * 2 - 1) * STREAM.DENSITY_VARIANCE;
                    const densityDiff = targetDensity - currentDensity;
                    const creationProbability = Math.max(
                        0.2,
                        Math.min(0.95, 0.6 + densityDiff * 1.5),
                    );
                    ...
                    if (Math.random() < creationProbability) {
                        ...
                        streams.forEach((existingStream) => {
                            ...
                            const minAllowedDistance =
                                minSpacing * (0.7 + Math.random() * 0.3);
            ...
            streams.forEach((stream) => {
                ...
                stream.chars.forEach((char, index) => {
                    ...
                    const baseAlpha =
                        0.2 +
                        (stream.depthLevel / (STREAM.DEPTH_LEVELS - 1)) * 0.8;
                ...
                if (
                    stream.y >
                    height + STREAM.MAX_LENGTH * ANIMATION.FONT_SIZE_MAX
                ) {
