...
const result = hello();`);
    ...
    const darkThemeIds = allThemes
        .map((t) => t.id)
        .filter((id) => !lightThemeIds.includes(id));
...
console.log(msg);`;
    ...
    function detectLanguage(code) {
        ...
        if (
            /(?:function|const|let|var|=>|import|export|require\()/i.test(
                trimmed,
            )
        ) {
            if (
                /(?:interface|type\s+\w+\s*=|as\s+\w+|:\s*\w+\[\])/i.test(
                    trimmed,
                )
            ) {
                return "typescript";
        ...
        // Python patterns
        if (
            /(?:def\s+\w+|import\s+\w+|from\s+\w+\s+import|if\s+__name__\s*==|print\()/i.test(
                trimmed,
            )
        ) {
            return "python";
        ...
        if (
            /[.#]?\w+\s*\{[^}]*\}/i.test(trimmed) ||
            /@media|@import|@keyframes/i.test(trimmed)
        ) {
            return "css";
        }
        ...
        // Java patterns
        if (
            /(?:public\s+class|private\s+\w+|System\.out\.println)/i.test(
                trimmed,
            )
        ) {
            return "java";
        ...
        if (/(?:#include|int\s+main|printf\(|std::)/i.test(trimmed)) {
            return trimmed.includes("std::") || trimmed.includes("iostream")
                ? "cpp"
                : "c";
        }
        ...
        if (
            /^#!/.test(trimmed) ||
            /(?:echo|ls|cd|mkdir|chmod)\s+/i.test(trimmed)
        ) {
            return "bash";
        }
...
<div class="flex min-h-screen">
    ...
    <div
        class="w-80 bg-gray-50 border-r border-gray-200 flex flex-col sticky top-0 h-screen"
    >
        <div class="p-4 border-b border-gray-200">
            ...
            <div class="flex rounded-lg bg-gray-200 p-1">
                <button
                    class="flex-1 px-3 py-1.5 text-sm font-medium rounded-md transition-colors {themeFilter ===
                    'all'
        ...
        <div class="flex-1 overflow-y-auto p-4">
            <div class="space-y-3">
                {#each filteredThemes as themeId}
                    ...
                    <button
                        class="w-full p-3 border rounded-lg hover:shadow-sm transition-all duration-200 text-left bg-white {selectedTheme ===
                        themeId
                    ...
                    >
                        ...
                        <div class="theme-preview text-xs overflow-hidden">
                            ...
                            {:else}
                                <div
                                    class="bg-gray-100 p-2 rounded text-gray-500"
                                >
                                    Loading...
                                </div>
                            {/if}
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        >
                        <div
                            class="px-3 py-2 bg-gray-100 rounded-md text-sm text-gray-700 min-w-0"
                        >
                            <div class="font-medium truncate">
                                {allThemes.find((t) => t.id === selectedTheme)
                                    ?.name || selectedTheme}
                            </div>
                            <div
        ...
        <div class="flex-1 px-6 pt-4 pb-6">
            <div class="flex flex-col">
                <div
                    ...
                    style="background-color: {getThemeBackground(
                        selectedTheme,
                    )}; min-height: 200px;"
                >
                    ...
                    <!-- Semi-transparent textarea overlay -->
                    <textarea
                        id="code-input"
                        bind:this={textareaRef}
                        bind:value={code_sample}
                        placeholder="Enter your code here..."
                        class="relative w-full px-4 py-3 bg-transparent resize-none outline-none font-mono text-sm leading-relaxed border-none text-overlay"
                        style="background: transparent; caret-color: {lightThemeIds.includes(
                            selectedTheme,
                        )
                            ? 'black'
                            : 'white'}; min-height: 200px; position: relative; z-index: 1; color: {lightThemeIds.includes(
                            selectedTheme,
                        )
                            ? 'rgba(0,0,0,0.08)'
...
<style>
    ...
    .text-overlay {
        /* Very low opacity text that allows highlighting to show through */
        opacity: 1;
    }
    ...
    /* Focus state to make text more visible when editing */
    .text-overlay:focus {
